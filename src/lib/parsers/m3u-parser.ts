...
export async function parseM3U(playlist: string | URL): Promise<M3UParsedData> {
  try {
    ...
    if (typeof playlist === 'string') {
      // Check if string is a URL or content
    ...
    return parseM3UContent(playlistContent, source);
  } catch (error) {
...
function extractChannelName(line: string): string | undefined {
  // Try tvg-name attribute first
  let name = extractAttribute(line, 'tvg-name');

  if (!name) {
    // Fallback to value after comma - simpler approach
